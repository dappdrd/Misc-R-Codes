...
# Set paths
Dir <- "C:\\Users\\dappdrd\\Desktop\\Round 6 TAMM Prep\\"
paths = list(paste(Dir, "Valid2018_TAMM_StockData_Test.xlsx", sep=""),
             paste(Dir, "Valid2018_TAMM_Fisheries_Test.xlsx", sep=""),
             paste(Dir, "New TAMMs\\", sep=""))
...
StockData <- as.data.frame(array(NA, c(238,26)))
colnames(StockData) <- c("ID", seq(from=1992,to=2016,by=1))

FishData <- as.data.frame(array(NA, c(144, 26)))
colnames(FishData) <- c("ID", seq(from=1992,to=2016,by=1))
...
for(i in 1992:2016) {
    # Set TAMM file path
    TAMM <- paste(TAMM1, "Valid2018_", i, "_Rnd6_8.29.2018.xlsm",sep="")
    ...
    SPS128 <- Input_Page$Cells(55,20); SPS128[["Value"]] <- iData[238,2]
